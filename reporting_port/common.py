...
USAGE_FILE = DATA_DIR / "code_usage_combined_apcs.csv"
PREFIX_WARNINGS_FILE = DATA_DIR / "prefix_matching_repos.csv"
PREFIX_DETAILS_FILE = DATA_DIR / "prefix_matching_details.json"
GITHUB_CACHE_FILE = DATA_DIR / "github_code_search_cache.json"
SWAPPED_CODES_FILE = DATA_DIR / "swapped_codes.json"
CHANGED_CODE_DEFINITIONS_FILE = DATA_DIR / "changed_code_definitions.json"
PROJECT_TYPE_OVERRIDES_FILE = DATA_DIR / "project_type_overrides.json"
...
def load_usage_data(_data_source="apcs"):
...
def load_cache():
    if not GITHUB_CACHE_FILE.exists():
        return {}
    try:
        with GITHUB_CACHE_FILE.open() as f:
            return json.load(f)
...
def save_cache(cache):
    try:
        with GITHUB_CACHE_FILE.open("w") as f:
...
def find_all_codes_in_github(codes, force):
    ...
    cache = filter_github_search_results(
        {}
        if force
        else {code: value for code, value in load_cache().items() if code in codes}
    )
    if cache:
...
def load_project_type_overrides():
...
def filter_cohortextractor_moved_codes(all_results, groups, overrides=None):
    ...
    repos = {repo for repo_results in all_results.values() for repo in repo_results}
    project_types = {repo: get_project_type(repo, overrides) for repo in sorted(repos)}
    file_cache = {}
    filtered = {}
    for code, repo_results in all_results.items():
...
def load_changed_code_definitions():
    try:
        with CHANGED_CODE_DEFINITIONS_FILE.open() as f:
            entries = json.load(f)
    except (OSError, json.JSONDecodeError) as error:
